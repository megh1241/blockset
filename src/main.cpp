...
static void parseArgs(int argc, char* argv[]){
    ...
    {"--help", "--numbins", "--obsnum", "--mode", "--layout", "--intertwine", "--labelcol", "--logdir", "--numfiles", 
...
//        omp_set_dynamic(0);
 //       omp_set_num_threads(std::atoi(Config::getValue("numthreads").c_str()));
    ...
    if(Config::getValue("numfiles") == std::string("notfound")){
        Config::setConfigItem(std::string("numfiles"), std::string("1"));
    }
